...
def Strat2(symbol, qty, interval, entried=False):
    trend_data = []
    end_time = time.time() + 50*1  # How many minutes to build up the data for before starting to trade.
    ...
    if entried == True:
        while True:
            ...
            if len(sincebuy) > 0:
                ...
                if sincebuy_returns[-1] > 0.0015 or sincebuy_returns[-1] < -0.003:
                    order = client.order_market_sell(symbol=symbol, quantity=qty)
                    print(order)
                    break
        print('Algorithm Complete. Thank you for choosing Goodall Logistics Ltd.')
...
try:
    while time.time() < final_time:
        Strat2(symbol='BTCGBP', qty=0.001, interval='60')
        print('Cycled ETH')
except:
