...
eth = GetMinuteData('ETHGBP', '1m', '10m')
eth.Open.plot(title='ETH')

btc = GetMinuteData('BTCGBP', '1m', '10m')
btc.Close.plot(title='BTC / GBP')
...
# Working Strat
# Previous function - works as of 2022-10-10 and took roughly 2 minutes to complete a sell.
def Strat2(symbol, qty, interval, entried=False):
    trend_data = []
    end_time = time.time() + 40*1  # How many minutes to build up the data for before starting to trade.
    ...
    if entried == False:
        ...
        if average_trend > 1.0008:  # Change this value to alter the threshold
            order = client.order_market_buy(symbol=symbol,
                                            quantity=qty)
            print(order)
            entried = True
...
'''
    ...
    if entried == True:
        while True:
            ...
            if len(sincebuy) > 0:
                sincebuy_returns = (sincebuy.Close.pct_change() + 1).cumprod() - 1
                # Sell if asset rises by more than 0.1% (This is minimum as otherwise fees get us) OR falls again by 0.15%
                if sincebuy_returns[-1] > 0.001:  # or sincebuy_returns[-1] < -0.002:
                    order = client.order_market_sell(symbol=symbol, quantity=qty)
                    print(order)
                    break
        print('Algorithm Complete. Thank you for choosing Goodall Logistics Ltd.')
...
try:
    while time.time() < final_time:
        Strat2(symbol='ETHGBP', qty=0.01, interval='30')
        print('Cycled ETH')
except:
    print('Time Limit Reached. Program Closed.')
